...
s = set(map(int, input().split()))  # Initialize the set with elements
...
for _ in range(N):
    ...
    if operation == "pop":
        if s:  # Check if set is not empty before popping
            s.pop()
    elif operation == "remove":
        value = int(command[1])
        if value in s:  # Ensure the element exists before removing
            s.remove(value)
    elif operation == "discard":
        value = int(command[1])
        s.discard(value)  # No need to check existence
