...
a = list(map(int, input().split()))  # Convert to list to allow indexing
b = [list(map(float, input().split())) for _ in range(a[1])]  # Convert inner maps to lists

for i in zip(*b):  # Transpose the matrix
    print(sum(i) / a[1])  # Compute the average